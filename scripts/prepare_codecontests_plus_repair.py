...
def load_candidates(config: dict[str, Any]) -> tuple[list[dict[str, Any]], list[str]]:
    ...
    for index, shard in enumerate(shards, 1):
        ...
        with fs.open(f"datasets/{source['dataset']}/{shard}", "rb") as handle:
            ...
            rows.extend(parquet.read_row_groups(range(count), columns=columns).to_pylist())
    return stable_sample(rows, len(rows), int(selection["seed"])), shards
...
def main() -> int:
    ...
    candidates, shards = load_candidates(config)
    ...
    for candidate_index, row in enumerate(candidates, 1):
        ...
        if (
            ...
            rejected["quality_threshold"] += 1
            continue
        if not row.get("checker") or len(tests) < int(selection["minimum_tests"]):
            rejected["missing_checker_or_tests"] += 1
            continue
        ...
        if not correct_codes or not incorrect_codes:
            rejected["missing_cpp_submission_class"] += 1
            continue
        ...
        print(
            f"Accepted {len(accepted)}/{selection['target_count']} after {candidate_index} candidates: "
            f"{problem_id} ({initial_judge.error_type})",
            flush=True,
        )

    if len(accepted) < int(selection["target_count"]):
        raise RuntimeError(f"Only {len(accepted)} problems survived; need {selection['target_count']}")
