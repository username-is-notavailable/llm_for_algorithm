...
if [[ ! "${MODEL_SIZE}" =~ ^(1\.7b|1\.7b-post|4b|4b-post)$ ]] || [[ ! "${SPLIT}" =~ ^(smoke|dev)$ ]] || \
  ...
  echo "Usage: bash scripts/cloud_eval_agent.sh {1.7b|1.7b-post|4b|4b-post} {smoke|dev} {oneshot|agent|both|agent-sharded} [--resume OUTPUT_DIR]" >&2
  exit 2
fi
shift 3
...
run_agent_sharded() {
  ...
  case "${MODEL_SIZE}" in
    ...
    4b-post) base="m9-agent-qwen3-4b-posttrained-${SPLIT}-v1" ;;
  esac
